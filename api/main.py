...
async def start_research(request: ResearchRequest, background_tasks: BackgroundTasks):
    ...
    return ResearchResponse(
        task_id=task_id, status="started", result=None, error=None, process_summary=None
    )


async def execute_research_task(
    task_id: str, task_description: str, model_name: str, temperature: float, search_engine: str
):
    """在后台执行研究任务"""
    try:
        # 更新状态
        ...
        # 创建研究代理
        agent = DeepResearchAgent(llm=llm, verbose=False, callbacks=[callback_handler])
...
async def list_tasks():
    ...
    return {
        "tasks": [
            {"task_id": task_id, "status": info["status"], "created_at": info.get("created_at")}
...
async def get_config_info():
    ...
    return {
        ...
        "optional_env_vars": ["SEARCH_API_KEY", "GOOGLE_CX"],
    }
