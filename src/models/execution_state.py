...
class EnhancedExecutionState(BaseModel):
    ...
    decomposed_goals: List[Dict[str, Any]] = Field(
        default_factory=list
    )  # {id, description, status, created_at, completed_at, related_searches, related_knowledge}
    search_iterations: List[Dict[str, Any]] = Field(
        default_factory=list
    )  # {id, timestamp, query, results: List[SearchResultItem], goal_id}
    knowledge_base: List[KnowledgeItem] = Field(default_factory=list)  # List of KnowledgeItem
    hypotheses: List[Dict[str, Any]] = Field(
        default_factory=list
    )  # {id, hypothesis, status (pending_verification, supported, refuted), supporting_knowledge_ids, contradicting_knowledge_ids}
    contradictions: List[Dict[str, Any]] = Field(
        default_factory=list
    )  # {id, description, conflicting_knowledge_ids, resolution_attempt_description, status (unresolved, resolving, resolved)}
    ...
    def add_iteration(
        self, query: str, results: List[SearchResultItem], goal_id: Optional[str] = None
    ) -> str:
        """添加一次搜索迭代记录"""
        ...
        self.hypotheses.append(
            {
                ...
                "supporting_knowledge_ids": (
                    supporting_knowledge_ids if supporting_knowledge_ids else []
                ),
    ...
    def update_hypothesis_status(
        ...
        hyp_id: str,
        status: str,
        supporting_knowledge_ids: List[str] = None,
        contradicting_knowledge_ids: List[str] = None,
    ) -> bool:
        """更新假设的状态和相关知识"""
    ...
    def update_contradiction_status(
        self, contra_id: str, status: str, resolution_description: Optional[str] = None
    ) -> bool:
        """更新矛盾的状态和解决方案"""
    ...
    def get_knowledge_by_recency(self, days: int = 365) -> List[KnowledgeItem]:
        ...
        return sorted(
            recent_knowledge,
            key=lambda x: parse_date(x.publish_date) if x.publish_date else datetime.datetime.min,
            reverse=True,
        )
    ...
    def get_goal_progress(self) -> Dict[str, int]:
        """获取目标完成进度统计"""
        stats = {
            "total": len(self.decomposed_goals),
            "completed": 0,
            "in_progress": 0,
            "pending": 0,
        }
