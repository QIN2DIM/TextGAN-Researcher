...
class KnowledgeFreshnessAnalysisTool(BaseTool):
    ...
    def _run(self, time_period: str = "1年") -> str:
        ...
        for i, item in enumerate(recent_knowledge[:5], 1):
            report.append(
                f"{i}. {item.key} (来源: {item.source}, 日期: {item.publish_date if item.publish_date else '未知'})"
            )
            report.append(f"   可信度: {item.confidence*100:.1f}%")
            report.append(
                f"   内容: {str(item.value)[:100]}..."
                if len(str(item.value)) > 100
                else f"   内容: {item.value}"
            )
...
class ContradictionDetectionTool(BaseTool):
    ...
    def _run(self, knowledge_keys_json: str = "[]") -> str:
        try:
            ...
            if analysis_result.get("status") == "contradictions_found" and analysis_result.get(
                "contradictions"
            ):
                report = ["检测到以下矛盾:"]
...
class ResolveContradictionTool(BaseTool):
    ...
    def _run(self, input_json: str) -> str:
        try:
            ...
            contradiction = next(
                (c for c in self.state.contradictions if c["id"] == contra_id), None
            )
            if not contradiction:
                return f"未找到矛盾ID: {contra_id}"

            conflicting_knowledge_items = [
                self.state.get_knowledge_item(k)
                for k in contradiction["conflicting_keys"]
                if self.state.get_knowledge_item(k)
            ]
            ...
            self.state.update_contradiction_status(
                contra_id, resolution_result["status"], resolution_result["resolution_description"]
            )
...
class VerifyHypothesisTool(BaseTool):
    ...
    def _run(self, hypothesis_id: str) -> str:
        try:
            ...
            relevant_knowledge = [
                self.state.get_knowledge_item(k)
                for k in hypothesis["supporting_knowledge_ids"]
                if self.state.get_knowledge_item(k)
            ]
            relevant_knowledge.extend(
                [
                    self.state.get_knowledge_item(k)
                    for k in extracted_knowledge_keys
                    if self.state.get_knowledge_item(k)
                ]
            )
            ...
            search_results_str = "\n".join(
                [
                    f"- {sr.content[:100]}... (来源: {sr.url}, 日期: {sr.publish_date})"
                    for sr in search_results
                ]
            )
            ...
            self.state.update_hypothesis_status(
                ...
                contradicting_knowledge_ids=verification_result.get(
                    "contradicting_knowledge_keys", []
                ),
            )
