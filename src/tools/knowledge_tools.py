    ...
    def _run(self, input_json: str = "{}") -> str:
        try:
            ...
            for i, item in enumerate(knowledge_items[:limit]):
                summary.append(
                    f"{i+1}. {item.key} (来源: {item.source}, 日期: {item.publish_date if item.publish_date else '未知'}, 可信度: {item.confidence*100:.1f}%, 验证: {item.verified})"
                )
