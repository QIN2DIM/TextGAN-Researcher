...
class GenerateContentTool(BaseTool):
    ...
    def _run(self, purpose: str = "研究报告") -> str:
        ...
        self.agent_instance.callback_manager.on_tool_end(
            f"Generator produced content for '{purpose}'."
        )

        # Rewarder 评估内容
        evaluation = self.agent_instance.rewarder.evaluate(
            self.agent_instance.state.task_description, generated_content
        )
        self.agent_instance.callback_manager.on_tool_end(
            f"Rewarder scored content: {evaluation['score']}/10. Reason: {evaluation['reason']}"
        )
        ...
        self.agent_instance.callback_manager.on_tool_end(
            f"Reviewer provided criticism: {criticism[:150]}..."
        )
